...
| **E7** | (unstated) M2.5 needs source reconnaissance | Confirmed at n = 19, re-derived offline from the 47 cached pages: **29 exams publish a `Caderno de Prova - Tipo N`; 19 are 2019 or later; all 19 also publish a 1ª-fase gabarito**, and all 38 PDF URLs resolved on first request. Discovery needs one filter the census did not: the same page carries `Edital - Locais e Horário de Realização da Prova Objetiva (1ª fase)` and `Resultado Definitivo (após recursos) - Prova Objetiva (1ª fase)`, both of which match a naive gabarito pattern. | M2.5 is decoupled from the unbuilt "Source widening" milestone; discovery costs no network. Selection drops labels containing *edital / resultado / comunicado / local / horário / isenção / inscrição / recurso* **before** matching. **The consequence first claimed here — "or two administrative PDFs are fetched per exam instead of the answer key" — was never measured and is false.** Re-measured over all 45 cached index pages / 1,956 labels (2026-08-10, final review): the `_ADMIN` pre-filter changes **zero** selections, because `_GABARITO` (`gabaritos?\b.*prova\s+objetiva`) and `_CADERNO_TIPO` already reject every one of those labels on their own. The guard ships anyway — it is cheap, it states the intent, and a future label wording could need it — but it is defence in depth, not a fix for an observed failure, and it now carries an adversarial test (`ADVERSARIAL_ADMIN_*`) built from labels engineered to defeat the other two patterns rather than from anything real. |
...
**Superseded by the real run — the figure is 405, not 419. See § Result below; this
section is kept, corrected, for the reasoning it carries.**

Measured, not extrapolated (Task 1, 2026-08-09, then re-measured on the real run
2026-08-10). The 18 ingestible exams yield **1,439 segmented items**, of which the tuned
keyword gate keeps **405** — an average of 22.5 per exam, far above the 100–170 this plan
first guessed, because the gate is deliberately generous. (Task 1 said 419. That count
was taken with the pre-Task-2 tail-boundary defect in place, which spliced running heads
and the questionário de percepção into each exam's last items; the foreign text scored
keyword hits of its own. Nothing about the gate changed — its input did.) The hand check
on the 43º puts its precision at roughly **half**: 16 of the 30 items it kept there are
penal or processo penal to a domain reader. So expect on the order of **200 genuinely
criminal items** reaching `classify`, of which the processo-penal subset — the part this
course actually needs — is roughly **90–120**, before `vet` and `curate` cut further.
Measured after the run: `direito-processual-penal` rose by **119**, inside that band.

The 405 is the number that matters operationally: it is the classify bill. The ~200 is
...
3. **T3.3 ("Regimes de avaliação da prova e standards probatórios") is no longer empty.** It already
   ...
   shows that message when there are zero candidates. This made `CLAUDE.md`'s line "**T3.3** stays
   empty by design" stale; **corrected in the final fix wave** — the design decision (opened from
   doctrine, never counted as a gap) stands, the "empty" and "~770 questions searched" facts did
   not.
